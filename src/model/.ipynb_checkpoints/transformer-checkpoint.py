...
class TransformerDecoder(nn.Module):
	...
	def generate_beam(self, encoded, lang_id, beam_size=20, max_len=175, sample=False, temperature=None):
		...
		lengths0 = torch.LongTensor([sent[0]['tokens'].numel() for sent in results])
		lengths0.add_(1)  # for BOS
		max_len = lengths0.max()
		bsz = len(results)
		decoded0 = results[0][0]['tokens'].new(max_len, bsz).fill_(0)
		decoded0[0, :] = self.bos_index[lang_id]
		for i, sent in enumerate(results):
			ntoks = sent[0]['tokens'].numel()  # pick the top beam result
			decoded0[1:ntoks + 1, i] = sent[0]['tokens']
		
# 		lengths1 = torch.LongTensor([sent[1]['tokens'].numel() for sent in results])
# 		lengths1.add_(1)  # for BOS
# 		max_len = lengths1.max()
# 		bsz = len(results)
# 		decoded1 = results[0][0]['tokens'].new(max_len, bsz).fill_(0)
# 		decoded1[0, :] = self.bos_index[lang_id]
# 		for i, sent in enumerate(results):
# 			ntoks = sent[1]['tokens'].numel()  # pick the top beam result
# 			decoded1[1:ntoks + 1, i] = sent[1]['tokens']
		
# 		lengths2 = torch.LongTensor([sent[2]['tokens'].numel() for sent in results])
# 		lengths2.add_(1)  # for BOS
# 		max_len = lengths2.max()
# 		bsz = len(results)
# 		decoded2 = results[0][0]['tokens'].new(max_len, bsz).fill_(0)
# 		decoded2[0, :] = self.bos_index[lang_id]
# 		for i, sent in enumerate(results):
# 			ntoks = sent[2]['tokens'].numel()  # pick the top beam result
# 			decoded2[1:ntoks + 1, i] = sent[2]['tokens']
			
# 		lengths3 = torch.LongTensor([sent[3]['tokens'].numel() for sent in results])
# 		lengths3.add_(1)  # for BOS
# 		max_len = lengths3.max()
# 		bsz = len(results)
# 		decoded3 = results[0][0]['tokens'].new(max_len, bsz).fill_(0)
# 		decoded3[0, :] = self.bos_index[lang_id]
# 		for i, sent in enumerate(results):
# 			ntoks = sent[3]['tokens'].numel()  # pick the top beam result
# 			decoded3[1:ntoks + 1, i] = sent[3]['tokens']
			
# 		lengths4 = torch.LongTensor([sent[4]['tokens'].numel() for sent in results])
# 		lengths4.add_(1)  # for BOS
# 		max_len = lengths4.max()
# 		bsz = len(results)
# 		decoded4 = results[0][0]['tokens'].new(max_len, bsz).fill_(0)
# 		decoded4[0, :] = self.bos_index[lang_id]
# 		for i, sent in enumerate(results):
# 			ntoks = sent[4]['tokens'].numel()  # pick the top beam result
# 			decoded4[1:ntoks + 1, i] = sent[4]['tokens']


		return decoded0, lengths0#, decoded1, lengths1, decoded2, lengths2, decoded3, lengths3, decoded4, lengths4, one_hot
